...
def segment_polyline_intersects(a: Point, b: Point, polyline: List[Point]) -> bool:
    return any(
...
class WildSentinel098:
    """
    V0.9.8 safety-hardening reference engine.

    Key design rule:
        The engine does not require certainty to impose a safety bound,
        but it does require evidence before claiming a precise TTC.
    """

    def __init__(self):
        self.routes: Dict[str, RouteState] = {r: RouteState() for r in ROUTES}
    ...
    def inflate_uncertainty(self, base: float) -> float:
        ...
        # Persistent prediction error expands the safety envelope.
    ...
    def validate_prediction(self, actual: Point, predicted: Point):
    ...
    def hypotheses(self, visible: bool, evidence: Evidence, uncertainty: float):
        if visible:
            return {
    ...
    def classify_ttc(self, ttc: Optional[float], intersects: bool) -> str:
        if ttc is None:
            return "UNKNOWN"
        if not intersects:
    ...
    def route_distance(self, position: Point, route: str) -> float:
        return point_to_polyline_distance(
            position, self.route_geometry[route].points
        )
    ...
    def encounter_exposure(
        ...
        center_distance = point_to_polyline_distance(
            predicted_position, geometry.points
        )

        envelope_radius = uncertainty * 2.0
        overlap = max(0.0, envelope_radius - center_distance)
        spatial_overlap = clamp(
            overlap / max(1.0, envelope_radius) * 100.0
        )

        intersects = False
        if trajectory_start is not None and trajectory_end is not None:
            intersects = segment_polyline_intersects(
                trajectory_start, trajectory_end, geometry.points
            )

        envelope_intersects = center_distance <= envelope_radius

        if envelope_intersects:
            geometric_exposure = max(spatial_overlap, 25.0)
        else:
            distance_factor = max(
                0.0,
    ...
    def time_to_conflict(
        ...
        We estimate closing speed from distance-to-route change over the
        actual elapsed timestep. If distance is not decreasing, we do NOT
        invent a TTC.

        If the projected segment intersects the route, sampled crossing
        time is used. Otherwise the uncertainty envelope is checked.
        """

        geometry = self.route_geometry[route].points

        d0 = point_to_polyline_distance(trajectory_start, geometry)
        d1 = point_to_polyline_distance(trajectory_end, geometry)

        # Direct geometric crossing.
        if segment_polyline_intersects(
            trajectory_start, trajectory_end, geometry
        ):
            samples = 100
            for i in range(samples + 1):
                t = i / samples
                p = lerp(trajectory_start, trajectory_end, t)
                if point_to_polyline_distance(p, geometry) <= uncertainty:
                    return t * time_step_minutes, True

        # Already inside uncertainty envelope.
        if d0 <= uncertainty:
            return 0.0, True

        # Projected endpoint enters the uncertainty envelope.
        if d1 <= uncertainty and d0 > d1:
            closing_distance = d0 - uncertainty
            distance_closed = max(0.001, d0 - d1)
            ttc = time_step_minutes * closing_distance / distance_closed
            return max(0.0, ttc), True

        # No closing movement => no defensible TTC.
        delta_distance = d0 - d1
        if delta_distance <= 0:
            return None, False

        closing_speed = delta_distance / max(0.001, time_step_minutes)

        # Estimate time to route centreline.
        ttc = d0 / max(0.001, closing_speed)
    ...
    def update_route(
        ...
        # --------------------------------------------------------------
        # SPATIAL SAFETY RECORD VALIDATION
        #
        # Never allow incomplete spatial data to be interpreted
        # as a safe condition.
        # --------------------------------------------------------------
        ...
        return {
            ...
            "encounter_exposure": spatial["exposure"],
    ...
    def decide(self, route_results: Dict[str, dict], confidence_context: float):
        ranked = sorted(route_results.items(), key=lambda kv: kv[1]["temporal"])
        ...
        if current["temporal"] >= self.danger_threshold:
            if (
                ...
                return (
                    ...
                    {
                        "reason": (
                            "Emergency route switch: current route exceeded "
            ...
            return (
                ...
                {
                    "reason": (
                        "Current route is unsafe and no safer route is available."
                    ),
        ...
        if (
            current["ttc_state"] in ("IMMEDIATE", "IMMINENT")
            ...
            if (
                ...
                return (
                    best_route,
                    {
                        "reason": "Emergency route switch: imminent wildlife-route conflict.",
        ...
        if (
            ...
            best_route != self.selected_route
            and best["temporal"] + self.route_switch_margin < current["temporal"]
        ):
            chosen = best_route
    ...
    def validate_decision(self, chosen: str, route_results: Dict[str, dict]) -> List[str]:
        violations = []
        ...
        if chosen in route_results:
            ...
            if selected["classification"] == "DO NOT ENTER":
                ...
                if not all_unsafe:
                    violations.append(
                        "Selected route is DO NOT ENTER while a safer route exists."
                    )
        ...
        if chosen in route_results:
            ...
            if (
                ...
                violations.append(
                    "Unsafe selected route survived despite a safe alternative."
                )
...
def run_adversarial_tests(seed: int = 982026):
    print("=" * 78)
    print("                 WILD SENTINEL V0.9.8")
    ...
    e.selected_route = "C"
    results = {
        "A": {"temporal": 48, "classification": "CAUTION",
              "ttc_state": "DISTANT", "encounter_exposure": 20},
        "B": {"temporal": 47, "classification": "CAUTION",
              "ttc_state": "DISTANT", "encounter_exposure": 20},
        "C": {"temporal": 48, "classification": "CAUTION",
              "ttc_state": "DISTANT", "encounter_exposure": 20},
    }
    chosen, _ = e.decide(results, 80)
    check("Test 1: Normal hysteresis retention", chosen == "C")

    # 2
    e = WildSentinel098()
    e.selected_route = "C"
    results = {
        "A": {"temporal": 45, "classification": "CAUTION",
              "ttc_state": "DISTANT", "encounter_exposure": 20},
        "B": {"temporal": 50, "classification": "CAUTION",
              "ttc_state": "DISTANT", "encounter_exposure": 20},
        "C": {"temporal": 75, "classification": "DO NOT ENTER",
              "ttc_state": "IMMINENT", "encounter_exposure": 70},
    }
    chosen, decision = e.decide(results, 30)
    check(
        "Test 2: Unsafe current route emergency switch",
        chosen == "A" and decision["reason_code"] == "CURRENT_ROUTE_UNSAFE"
    )
    ...
    e.selected_route = "C"
    results = {
        r: {"temporal": risk, "classification": "DO NOT ENTER",
            "ttc_state": "IMMEDIATE", "encounter_exposure": risk}
        for r, risk in {"A": 90, "B": 80, "C": 75}.items()
    }
    ...
    e.selected_route = "C"
    results = {
        "A": {"temporal": 35, "classification": "CAUTION",
              "ttc_state": "DISTANT", "encounter_exposure": 10},
        "B": {"temporal": 40, "classification": "CAUTION",
              "ttc_state": "APPROACHING", "encounter_exposure": 25},
        "C": {"temporal": 70, "classification": "DO NOT ENTER",
              "ttc_state": "IMMINENT", "encounter_exposure": 75},
    }
    ...
    e.selected_route = "C"
    results = {
        "A": {"temporal": 60.4, "classification": "CAUTION",
              "ttc_state": "UNKNOWN", "encounter_exposure": 31.4},
        "B": {"temporal": 60.4, "classification": "CAUTION",
              "ttc_state": "UNKNOWN", "encounter_exposure": 31.4},
        "C": {"temporal": 67.4, "classification": "DO NOT ENTER",
              "ttc_state": "APPROACHING", "encounter_exposure": 31.4},
    }
    ...
    # 7: disappearance beside route
    e = WildSentinel098()
    spatial = e.encounter_exposure((50, 54), 8, "A", (50, 54), (58, 60))
    check(
        "Test 7: Sensor-blind uncertainty envelope preserved",
        spatial["envelope_intersection"] and spatial["exposure"] > 0
    )

    # 8: boundary disappearance
    e = WildSentinel098()
    spatial = e.encounter_exposure((50, 59), 5, "A", (50, 59), (58, 51))
    check(
        "Test 8: Boundary uncertainty remains exposed",
        spatial["envelope_intersection"] or spatial["trajectory_intersection"]
    )

    # 9: prediction error inflation
    e = WildSentinel098()
    e.prediction_error_history = [20, 25, 30, 25, 20]
    ...
    state = e.classify_ttc(ttc, intersects)
    check(
        "Test 10: TTC can become UNKNOWN",
        state in ("UNKNOWN", "NO INTERSECTION", "DISTANT")
    )
    ...
    for a, b, c in [
        ...
        results = {
            "A": {"temporal": a, "classification": e.classify(a),
                  "ttc_state": "DISTANT", "encounter_exposure": 10},
            "B": {"temporal": b, "classification": e.classify(b),
                  "ttc_state": "DISTANT", "encounter_exposure": 10},
            "C": {"temporal": c, "classification": e.classify(c),
                  "ttc_state": "DISTANT", "encounter_exposure": 10},
        }
        sequence.append(e.decide(results, 70)[0])
    check(
    ...
    ttc, hit = e.time_to_conflict((20, 40), (20, 60), "A", 1, 5)
    check("Test 12: Crossing route detected", hit and ttc is not None)

    # 13 parallel geometry / moving away
    e = WildSentinel098()
    ttc, hit = e.time_to_conflict((20, 40), (30, 40), "A", 1, 5)
    check("Test 13: Parallel trajectory does not create false conflict",
          not hit or ttc is None)

    # 14 multi-segment route
    e = WildSentinel098()
    e.route_geometry["A"] = RouteGeometry(
        "A", [(0, 50), (75, 50), (75, 70), (150, 70)], 1.0
    )
    spatial = e.encounter_exposure((75, 60), 4, "A", (75, 40), (75, 65))
    check(
        "Test 14: Multi-segment route geometry works",
        spatial["trajectory_intersection"] or spatial["envelope_intersection"]
    )

    # 15 unsafe + minimum hold
    e = WildSentinel098()
    e.selected_route = "C"
    e.hold_counter = 0
    results = {
        "A": {"temporal": 50, "classification": "CAUTION",
              "ttc_state": "DISTANT", "encounter_exposure": 10},
        "B": {"temporal": 52, "classification": "CAUTION",
              "ttc_state": "DISTANT", "encounter_exposure": 10},
        "C": {"temporal": 66, "classification": "DO NOT ENTER",
              "ttc_state": "Distant", "encounter_exposure": 10},
    }
    ...
    # 16 low-confidence safe-looking route
    e = WildSentinel098()
    predicted, uncertainty = e.predict((50, 50), 20)
    check(
        "Test 16: Low confidence produces large uncertainty",
        uncertainty >= 25
    )

    # 17 every route unsafe
    e = WildSentinel098()
    results = {
        r: {"temporal": 65 + i, "classification": "DO NOT ENTER",
            "ttc_state": "UNKNOWN", "encounter_exposure": 60}
        for i, r in enumerate(ROUTES)
    }
    chosen, decision = e.decide(results, 5)
    check(
    ...
    check("Test 18: Genuine recovery reduces retained risk",
          result["temporal"] <= 14)

    # 19 uncertain recovery should not become LOW merely because TTC unknown
    e = WildSentinel098()
    state = e.routes["A"]
    state.memory = 60
    state.corridor_memory = 50
    ...
    check("Test 19: High uncertainty blocks false recovery",
          result["temporal"] >= 35)

    # 20 deterministic fuzz safety invariant
    rng = random.Random(seed)
    invariant_failures = 0

    for _ in range(5000):
        e = WildSentinel098()
        e.selected_route = rng.choice(list(ROUTES))
        ...
        for r in ROUTES:
            ...
            results[r] = {
                ...
                "ttc_state": rng.choice(
                    ["UNKNOWN", "DISTANT", "APPROACHING", "IMMINENT", "IMMEDIATE"]
                ),
                "encounter_exposure": rng.uniform(0, 100),
            }

        chosen, decision = e.decide(results, rng.uniform(0, 100))
        violations = e.validate_decision(chosen, results)
    ...
    if failed == 0:
        print("  >>> V0.9.8 ADVERSARIAL SAFETY TESTS PASSED")
    else:
        print("  >>> V0.9.8 REQUIRES FURTHER INVESTIGATION")
...
def run_demo():
    engine = WildSentinel098()
    print("\n" + "=" * 78)
    print("                 WILD SENTINEL V0.9.8 DEMO")
    print("=" * 78)

    timeline = [
        (0, (15.0, 85.0), False, Evidence(), {"A": (31, 56), "B": (6, 6), "C": (6, 6)}),
        (5, (23.0, 75.0), False, Evidence(), {"A": (20, 55), "B": (8, 8), "C": (6, 6)}),
        (10, (31.0, 65.0), True, Evidence(sensor=70), {"A": (40, 81), "B": (8, 31), "C": (6, 6)}),
        (15, (39.0, 55.0), True, Evidence(movement=65, sensor=75), {"A": (60, 84), "B": (10, 10), "C": (6, 6)}),
        (20, (47.0, 50.0), False, Evidence(movement=40, sensor=35), {"A": (65, 84), "B": (15, 15), "C": (6, 6)}),
        (25, (55.0, 48.0), False, Evidence(movement=50, sensor=45, prey=55), {"A": (70, 85), "B": (15, 85), "C": (6, 6)}),
        (30, (63.0, 49.0), False, Evidence(movement=65, sensor=60, prey=60, environment=50, human=70), {"A": (75, 90), "B": (20, 90), "C": (6, 6)}),
        (35, (71.0, 50.0), False, Evidence(movement=70, sensor=65, prey=55, environment=60, human=70), {"A": (80, 95), "B": (25, 95), "C": (6, 6)}),
        (40, (79.0, 51.0), False, Evidence(movement=75, sensor=70, prey=55, environment=60, human=70), {"A": (85, 95), "B": (25, 95), "C": (6, 6)}),
        (50, (95.0, 75.0), False, Evidence(movement=15, sensor=10, prey=5), {"A": (25, 85), "B": (10, 75), "C": (6, 6)}),
        (60, (110.0, 90.0), False, Evidence(), {"A": (12, 75), "B": (8, 70), "C": (6, 6)}),
        (75, (130.0, 105.0), False, Evidence(), {"A": (6, 60), "B": (6, 60), "C": (6, 67)}),
    ]
    ...
    for minute, actual, visible, evidence, risks in timeline:
        ...
        if visible:
            ...
            predicted, uncertainty = engine.predict(previous or actual, confidence)

        if visible:
            error, accuracy = engine.validate_prediction(actual, predicted)
        else:
            error = distance(actual, predicted)
            accuracy = clamp(100 - error * 4)
        ...
        step = 5.0 if previous_time is None else max(1.0, minute - previous_time)

        for route in ROUTES:
            current, worst = risks[route]
            spatial = engine.encounter_exposure(
                predicted, uncertainty, route, start, predicted
            )
            ttc, hit = engine.time_to_conflict(
                start, predicted, route, uncertainty, step
            )
            ttc_state = engine.classify_ttc(ttc, hit)

            route_results[route] = engine.update_route(
                route, current, worst, uncertainty, confidence,
                evidence, contradiction, spatial, ttc, ttc_state
            )
        ...
        violations = engine.validate_decision(chosen, route_results)

        print(f"\n{minute:02d} min | sensor={'VISIBLE' if visible else 'BLIND'}")
        print(f"  actual={actual} predicted={tuple(round(x,1) for x in predicted)}")
        print(f"  confidence={confidence:.1f}% uncertainty={uncertainty:.1f}")
        print(f"  route risks: " + " | ".join(
        ...
        if chosen == "SAFETY OVERRIDE":
            print(f"  >>> SAFETY OVERRIDE [{decision['reason_code']}]")
        elif decision["reason_code"] in ("CURRENT_ROUTE_UNSAFE", "IMMINENT_CONFLICT"):
            print(
                f"  >>> EMERGENCY SWITCH {before} -> {chosen} "
                f"[{decision['reason_code']}]"
            )
        else:
            print(f"  >>> ROUTE {chosen} [{decision['reason_code']}]")

        if violations:
            print("  !!! INVARIANT FAILURE:", violations)

        previous = actual
        previous_time = minute

    print("\n" + "=" * 78)
    print("V0.9.8 DEMO COMPLETE")
    print("=" * 78)
